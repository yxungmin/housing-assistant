/**
 * 아이콘 미리보기 HTML. `src/components/icons.ts`의 도형 데이터를 그대로 그려서,
 * 눈으로 확인한 모양과 앱에 나가는 모양이 갈리지 않게 한다.
 * `npx tsx scripts/icon-preview.ts` → scripts/icon-preview.html
 */
import { writeFileSync } from "node:fs";
import { join } from "node:path";
import { ICONS, ICON_NAMES, type Shape } from "../src/components/icons";
...
/** 다듬는 중인 것만 크게 본다 */
const FOCUS = ["settings", "bus", "subway", "store", "tree", "school", "cart", "baby", "walk"];
const focusGrid = `<h2>검토 (96px)</h2><div class="grid">${FOCUS.map((n) => cell(n, 96, 2)).join("")}</div>`;
...
</style>${focusGrid}${grid("48px (크게)", 48, 2)}${grid("24px (실제 사용 크기)", 24, 2)}${grid("16px (가장 작게 쓰는 크기)", 16, 2.2)}`;
