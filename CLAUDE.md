...
- 매입임대·전세임대는 집이 흩어져 있다. 주택별 소재지·면적·임대조건은 공고문이 아니라 별도 엑셀 첨부에 있고,
  ...
  좌표는 주소마다 한 번만 찍는다(한 건물에 여러 세대). 그 호출이 역·정류장·주변 시설까지 같이 준다.
  **번들 파일에서는 주소별 표(`unit_places`, `packages/schema/src/units.ts`)로 접어 둔다** — 집마다 사본을 들면 번들의 82%가 그 사본이었다(2026-09-24).
  앱은 읽을 때 `inflateAnnouncement`로 집마다 다시 붙이고, DB의 `units`는 펼친 채다(Edge Function `transit`이 `unit.lat`을 읽는다). `push-bundle`이 올릴 때 펼친다.
...
- 수집기 클라이언트의 기본 fetch는 `collector/src/http.ts`의 `resilientFetch()`다 — 시간 제한 15초, 5xx·429·네트워크 오류 2회 재시도. 테스트가 넘기는 가짜 fetch는 그대로 통한다.
