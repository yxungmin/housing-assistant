...
3. `npm run fixture:check -- <id>`로 1차 대조한다. 룰·가격마다 이렇게 본다.
   - 인용문이 `source.page`에 실제로 있는가 (공백·줄바꿈 무시)
   - 없다면 다른 쪽에 있는가 / 표의 한 행을 문장으로 재구성한 것인가 / 원문에 아예 없는가
   - 값이 인용문과 맞는가 — `4,542만원` ↔ `45420000`, 가격표의 `514,020(천원)` ↔ `514020000`까지 맞춰 본다
   결과는 쪽 단위로 묶여 나온다. 공고문 그 쪽을 펴 놓고 한 번에 확인하면 된다.
   **기계 통과는 "원문에 그 문장이 있고 숫자가 그 문장과 맞는다"는 뜻일 뿐이다.** 조건을 옳게 해석했는지는 사람만 안다.
4. 다 확인했으면 `npm run fixture:check -- <id> --promote --reviewed-by <이름>`.
   초안이 `fixtures/<id>.json`으로 옮겨지고 룰의 `verified`가 true가 된다. 기계가 문제로 본 항목이 남아 있으면 거부한다.
5. `npm run benchmark -- --only <id>`로 그 건만 돌려 본다.
